    ...
    <motion.footer ref={ref}
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <motion.p
          ...
          className="font-mono text-xs" style={{ color: '#444444' }}>
          Designed &amp; built by{' '}
          <span style={{ color: '#7C5CFC' }}>Milhara Bhagya</span> · 2025
        </motion.p>
        <div className="flex gap-6">
          {[
            ...
            <motion.a key={item.label} href={item.href} target="_blank" rel="noreferrer"
              ...
              whileHover={{ color: '#7C5CFC', y: -3 }}
              className="text-xs" style={{ color: '#444444', transition: 'all 0.25s ease' }}>
              {item.label}
            </motion.a>
          ))}
        </div>
        <motion.button
          ...
          whileHover={{ scale: 1.15, borderColor: '#7C5CFC', color: '#7C5CFC', boxShadow: '0 4px 16px rgba(124,92,252,0.25)', y: -2 }}
          whileTap={{ scale: 0.9 }}
          className="w-9 h-9 rounded-full border flex items-center justify-center text-sm"
          style={{ borderColor: '#333333', color: '#444444', transition: 'all 0.3s ease' }}>
          ↑
        </motion.button>
      </div>
    </motion.footer>
